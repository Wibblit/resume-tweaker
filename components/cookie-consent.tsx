...
export function CookieConsent({
  ...
  const deleteNonEssentialCookies = () => {
    const expiryDate = getExpiryDate(1, 1, 4);

    document.cookie.split(";").forEach((cookie) => {
      const cookieName = cookie.split("=")[0].trim();
      if (cookieName !== "essentialCookies") {
  ...
  const accept = () => {
    const expiryDate = getExpiryDate(1, 1, 4);
    setIsOpen(false);
    document.cookie = `cookieConsent=true; expires=${expiryDate};`;
    setTimeout(() => {
      setHide(true);
    }, 700);
    onAcceptCallback();
  ...
  const decline = () => {
    ...
    document.cookie = `essentialCookies=true; expires=${expiryDate}; path=/`;
