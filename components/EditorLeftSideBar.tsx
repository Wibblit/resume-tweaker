...
import LeftSidePanel from "./LeftSidePanel";
import { useAppDispatch, useAppSelector } from "@/hooks/hooks";
import type { Skill, URL, ResumeData, ResumeSection } from "@/types/types";
import { RichInput } from "./TextEditor";
import { useMediaQuery } from "react-responsive";
import { CustomDatePicker } from "@/components/DatePicker";
import {
...
export default function LeftSideBar({
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      formData.append("file", file);
      if (resumeData.basics) {
        const prev = resumeData.basics[0].picture;
        const response = await axios.post(
          "/api/resumeimages-file-ops",
          formData
        );
  ...
  // New function to move an entry up in the section
  const moveEntryUp = (section: keyof ResumeData | string, id: string) => {
    const updatedResumeData = { ...resumeData };
    const sectionData = updatedResumeData[section as keyof ResumeData];
    ...
    const index = sectionData.findIndex((entry: any) => entry.id === id);
    ...
    //@ts-ignore
    updatedResumeData[section as keyof ResumeData] = newArray;
    dispatch(UpdateLeftBarData(updatedResumeData));
  };

  // New function to move an entry down in the section
  const moveEntryDown = (section: keyof ResumeData | string, id: string) => {
    const updatedResumeData = { ...resumeData };
    const sectionData = updatedResumeData[section as keyof ResumeData];
    ...
    const index = sectionData.findIndex((entry: any) => entry.id === id);
    if (index === -1 || index >= sectionData.length - 1) return; // Already at the bottom
  ...
  // New functions to move skills up and down
  const moveSkillUp = (entryId: string, skillIndex: number) => {
    if (skillIndex <= 0) return; // Already at the top

    const updatedResumeData = { ...resumeData };
    updatedResumeData.skills = updatedResumeData.skills?.map((entry) => {
      if (entry.id === entryId) {
        const newSkills = [...entry.skills];
        [newSkills[skillIndex], newSkills[skillIndex - 1]] = [
          newSkills[skillIndex - 1],
  ...
  const moveSkillDown = (entryId: string, skillIndex: number) => {
    const updatedResumeData = { ...resumeData };

    const entry = updatedResumeData.skills?.find((e) => e.id === entryId);
    if (!entry || skillIndex >= entry.skills.length - 1) return; // Already at the bottom
  ...
  const renderEntryFields = (
    ...
    const fields = resumeSections.find((s) => s.id === section)?.fields || [];
    return (
      <div key={entry.id} className="mb-8 border p-4 rounded-md relative">
        {/* Reordering controls */}
        {section !== "basics" && section !== "summary" && totalEntries > 1 && (
          <div className="absolute right-2 top-2 flex space-x-1">
            <Button
              variant="ghost"
              size="sm"
              onClick={() => moveEntryUp(section, entry.id)}
              disabled={index === 0}
            ...
            <Button
              variant="ghost"
              size="sm"
              onClick={() => moveEntryDown(section, entry.id)}
                ...
                <div className="space-y-4">
                  {(entry.skills || []).map(
                    (skill: Skill, skillIndex: number) => (
                      <div
                        key={skillIndex}
                        className="flex items-center space-x-2 mb-2 border p-2 rounded-md relative"
                      >
                        ...
                        <div className="flex gap-1">
                          <Button
                            variant="ghost"
                            size="sm"
                            onClick={() => moveSkillUp(entry.id, skillIndex)}
                          ...
                          <Button
                            variant="ghost"
                            size="sm"
                            onClick={() => moveSkillDown(entry.id, skillIndex)}
                            disabled={skillIndex === entry.skills.length - 1}
                            className="h-8 w-4 p-0"
                          >
                            <MoveDown className="h-4 w-4" />
                          </Button>
    ...
    return (
      <div className="flex flex-col h-full">
        <ScrollArea className="flex-grow my-8">
          ...
          {sectionEntries.map((entry, index) =>
            renderEntryFields(section, entry, index, sectionEntries.length)
          )}
        </ScrollArea>
        {section !== "basics" && section !== "summary" && (
          <div className="mt-4 space-y-2 mb-12">
                    ...
                    <AlertDialog>
                      ...
                      <AlertDialogContent>
                        <AlertDialogHeader>
                          <AlertDialogTitle>Clear All Data</AlertDialogTitle>
                          <AlertDialogDescription>
                            This action will clear all your resume data,{" "}
                            <strong className="font-bold">
                              Custom Sections cannot be retrieved once cleared.
                            </strong>{" "}
