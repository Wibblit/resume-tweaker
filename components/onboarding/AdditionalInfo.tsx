...
export default function AdditionalInfo({ updateFormData, formData }: AdditionalInfoProps): JSX.Element {
  ...
  const updateLanguage = (index: number, field: string, value: string): void => {
  ...
  const addVolunteer = (): void => {
    updateFormData({ volunteer: [...volunteer, { organization: '', role: '', startDate: '', endDate: '', location: '' }] })
  }

  const updateVolunteer = (index: number, field: string, value: string): void => {
  ...
  const addAward = (): void => {
    updateFormData({ awards: [...awards, { title: '', awarder: '', date: '', summary: '' }] })
  }
  ...
  const addPublication = (): void => {
    updateFormData({ publications: [...publications, { name: '', publisher: '', publishedIn: '', url: { href: '', label: '' }, date: '' }] })
  }

  const updatePublication = (index: number, field: string, value: string): void => {
    const updatedPublications = [...publications]
    if (field.startsWith('url.')) {
      updatedPublications[index].url = { ...updatedPublications[index].url, [field.split('.')[1]]: value }
    } else {
  ...
  const removePublication = (index: number): void => {
    updateFormData({ publications: publications.filter((_, i) => i !== index) })
  }

  const addCertification = (): void => {
    updateFormData({ certifications: [...certifications, { name: '', issuer: '', date: '', url: { href: '', label: '' } }] })
  }

  const updateCertification = (index: number, field: string, value: string): void => {
    const updatedCertifications = [...certifications]
    if (field.startsWith('url.')) {
      updatedCertifications[index].url = { ...updatedCertifications[index].url, [field.split('.')[1]]: value }
    } else {
  ...
  const addReference = (): void => {
    updateFormData({ references: [...references, { name: '', phone: '', email: '' }] })
  }
      ...
      <div>
        ...
        {volunteer.map((vol, index) => (
          <div key={index} className="mb-4 p-4 border rounded-md">
            ...
            <Button variant="ghost" size="icon" onClick={() => removeVolunteer(index)} className="mt-2">
              <Trash2 className="h-4 w-4" />
            </Button>
          </div>
        ))}
      ...
      <div>
        ...
        {awards.map((award, index) => (
          <div key={index} className="mb-4 p-4 border rounded-md">
            ...
            <Button variant="ghost" size="icon" onClick={() => removeAward(index)} className="mt-2">
              <Trash2 className="h-4 w-4" />
            </Button>
          </div>
        ))}
      ...
      <div>
        ...
        {publications.map((pub, index) => (
          <div key={index} className="mb-4 p-4 border rounded-md">
            ...
            <Button variant="ghost" size="icon" onClick={() => removePublication(index)} className="mt-2">
              <Trash2 className="h-4 w-4" />
            </Button>
          </div>
        ))}
      ...
      <div>
        ...
        {certifications.map((cert, index) => (
          <div key={index} className="mb-4 p-4 border rounded-md">
            ...
            <Button variant="ghost" size="icon" onClick={() => removeCertification(index)} className="mt-2">
              <Trash2 className="h-4 w-4" />
            </Button>
          </div>
        ))}
      ...
      <div>
        ...
        {references.map((ref, index) => (
          <div key={index} className="mb-4 p-4 border rounded-md">
            ...
            <Button variant="ghost" size="icon" onClick={() => removeReference(index)} className="mt-2">
              <Trash2 className="h-4 w-4" />
            </Button>
          </div>
        ))}
